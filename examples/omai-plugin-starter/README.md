...
Scaffold for an opinionated downstream plugin that layers proof capture and UI-TARS-driven vision testing onto `@taskflow-corp/cli`. This starter ships stubs; the real plugin implements screen capture and UI-TARS orchestration.
...
- `declare module '@taskflow-corp/cli/core'` augmentation so TypeScript knows about `ctx.plugins.omai`.
...
## Why it lives in the @taskflow-corp/cli repo as an example

This is a scaffold, not a published package. When you're ready to ship, copy it to a new repo `omai-taskflow` and publish as a separate npm package that depends on `@taskflow-corp/cli`.
